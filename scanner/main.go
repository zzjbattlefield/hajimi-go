...
func main() {
	...
	// 初始化 GitHub 客户端
	client := githubclient.NewClient(cfg.GithubTokens)
	...
	// 以追加模式打开文件，如果不存在则创建
	saveFile, err = os.OpenFile("data/valid_keys.txt", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		panic(err)
	}
	...
	if cfg.QueriesFile != "" {
		// 从文件读取查询
		content, err := os.ReadFile(cfg.QueriesFile)
		if err != nil {
	...
	// 初始化检查点管理器
	checkpointManager := checkpoint.NewManager(cfg.DataPath, querys)
	// 确保在退出时保存检查点
	defer func() {
	...
	err = scanGitHub(client, extractor, secretValidator, checkpointManager, querys, logger)
	if err != nil {
...
func processCodeResults(ctx context.Context, codeResults []*github.CodeResult, extractor *extractor.Extractor, secretValidator validator.Validator, checkpointManager *checkpoint.Manager, query string, page int, totalResults int, processed *int, logger *logger.Logger, cp *checkpoint.Checkpoint) error {
	for _, codeResult := range codeResults {
		...
		if cp.LastRepo != "" && cp.LastFile != "" && cp.LastCommit != "" {
			if repoName == cp.LastRepo && filename == cp.LastFile && commit == cp.LastCommit {
				// 跳过此文件并继续处理下一个
				logger.Infof("跳过已处理的文件: %s/%s", repoName, filename)
				continue
			}
		}
		...
		// 提取密钥
		secrets := extractor.Extract(content, filename, repoName, commit)
		...
		for _, secret := range secrets {
			...
			if err != nil {
				logger.Errorf("验证密钥失败 - 类型: %s, 值: %s, ", secret.Type, secret.Value)
				// 即使验证失败也记录密钥
				logger.Infof("找到密钥 - 类型: %s, 值: %s, 文件: %s, 仓库: %s, 行: %d",
			...
			if validationResult.Valid {
				logger.Infof("找到有效密钥 - 类型: %s, 值: %s, 文件: %s, 仓库: %s, 行: %d, 详情: %s",
					secret.Type, secret.Value, secret.File, secret.Repo, secret.Line, validationResult.Details)
				saveFile.WriteString(secret.Value + "\n")
			} else {
				logger.Infof("找到无效密钥 - 类型: %s, 值: %s, 文件: %s, 仓库: %s, 行: %d, 错误: %s, 详情: %s",
		...
		if (*processed)%10 == 0 { // 每处理 10 个文件更新一次检查点
			...
			logger.Infof("已处理 %d 个文件，保存检查点", *processed)
		}
...
func scanGitHub(client *githubclient.Client, extractor *extractor.Extractor, secretValidator validator.Validator, checkpointManager *checkpoint.Manager, querys []string, logger *logger.Logger) error {
	...
	for _, query := range querys {
		...
		if cp.LastPage > 0 {
			...
			logger.Infof("从第 %d 页恢复扫描", startPage)
		}
		...
		totalResults := cp.TotalResults
		processed := cp.Processed
		logger.Infof("开始使用查询: %s 进行扫描", query)
		// 搜索代码
		...
		// 如果我们是从头开始则更新总结果数
		if totalResults == 0 && result.Total != nil {
			totalResults = *result.Total
			logger.Infof("找到 %d 个代码结果", totalResults)
		}

		// 处理结果
		if err := processCodeResults(ctx, result.CodeResults, extractor, secretValidator, checkpointManager, query, opts.Page, totalResults, &processed, logger, cp); err != nil {
			return err
		}

		// 处理分页
		for resp != nil && resp.NextPage > 0 {
			logger.Infof("处理第 %d 页", resp.NextPage)
		...
		// 更新最终检查点
		if err := checkpointManager.Update(query, opts.Page, "", "", "", totalResults, processed); err != nil {
			logger.Errorf("更新最终检查点失败: %v", err)
		}
	}
